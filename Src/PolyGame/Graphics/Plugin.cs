...
public class RenderPlugin : IPlugin
{
    public void Apply(App app)
    {
        ...
        app.World.World.Component<Matrix2D>().Member<float>("M11").Member<float>("M12").Member<float>("M21").Member<float>("M22").Member<float>("M31")
            .Member<float>("M32");

        app.World.World.Component<RectangleF>().Member<float>("X").Member<float>("Y").Member<float>("Width").Member<float>("Height");

        app.World.World.Component<Matrix>().Member<float>("M11").Member<float>("M12").Member<float>("M13").Member<float>("M14").Member<float>("M21")
        ...
        app.AddSystems(SystemConfigs.Of(new ComputeCameraSystem(), new QueueSprites(registry).InSet(RenderSets.Queue), new RendererSystem().InSet(RenderSets.Render)));
    }
}
