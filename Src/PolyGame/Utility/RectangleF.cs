...
public struct RectangleF : IEquatable<RectangleF>
{
    ...
    private static Matrix2D _tempMat, _transformMat;
    ...
    ///     otherwise.
    /// </returns>
    public bool Contains(Point value) => X <= value.X && value.X < X + Width && Y <= value.Y &&
                                         value.Y < Y + Height;
    ...
    ///     otherwise.
    /// </returns>
    public bool Contains(Vector2 value) => X <= value.X && value.X < X + Width && Y <= value.Y &&
                                           value.Y < Y + Height;
    ...
    /// </returns>
    public bool Contains(RectangleF value) => X <= value.X && value.X + value.Width <= X + Width &&
    ...
    public float? RayIntersects(Ray ray)
    {
        ...
        {
            var num11 = 1f / ray.Direction.X;
            var num8 = (Left - ray.Position.X) * num11;
            var num7 = (Right - ray.Position.X) * num11;
            if (num8 > num7)
            {
                var num14 = num8;
                num8 = num7;
        ...
        {
            var num10 = 1f / ray.Direction.Y;
            var num6 = (Top - ray.Position.Y) * num10;
            var num5 = (Bottom - ray.Position.Y) * num10;
            if (num6 > num5)
            {
                var num13 = num6;
                num6 = num5;
    ...
    )
    {
        ...
        else
        {
            ...
            topLeft = _transformMat.Transform(topLeft);
            topRight = _transformMat.Transform(topRight);
            bottomLeft = _transformMat.Transform(bottomLeft);
            bottomRight = _transformMat.Transform(bottomRight);
    ...
    /// </summary>
    /// <returns>
    ///     The amount of overlap between two intersecting rectangles. These depth values can be negative depending on which
    ///     sides the rectangles
