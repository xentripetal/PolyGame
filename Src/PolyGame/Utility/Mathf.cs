...
public static class Mathf
{
    ...
    {
        var x = Sin(time * xFrequency + phase) * xMagnitude;
        var y = Cos(time * yFrequency) * yMagnitude;
    ...
    {
        var wrappedTime = PingPong(time, oscillationInterval);
        var damped = Pow(MathHelper.E, -damping * wrappedTime);

        var x = damped * Sin(time * xFrequency + phase) * xMagnitude;
        var y = damped * Cos(time * yFrequency) * yMagnitude;
