...
public class ImGuiRenderer
{
    ...
    // Textures
    private Dictionary<ulong, Texture2D> _loadedTextures = new();

    private ulong _textureId;
    private ImTextureID? _fontTextureId;

    public readonly VertexDeclaration VertDeclaration;

    public readonly int VertSize;
    ...
    public ImGuiRenderer(Game game)
    {
        unsafe
        {
            VertSize = sizeof(ImDrawVert);
        }

        VertDeclaration = new VertexDeclaration(
            VertSize,
            // Position
    ...
    public virtual unsafe void RebuildFontAtlas()
    {
        ...
        int height;
        int bytesPerPixel;
        ImGui.GetTexDataAsRGBA32(io.Fonts, &pixelData, &width, &height, &bytesPerPixel);
        ...
        if (_fontTextureId.HasValue) UnbindTexture(_fontTextureId.Value.Handle);
    ...
    private unsafe void UpdateBuffers(ImDrawDataPtr drawData)
    {
        ...
        if (drawData.TotalVtxCount > _vertexBufferSize)
        {
            ...
            _vertexBuffer = new VertexBuffer(_graphicsDevice, VertDeclaration, _vertexBufferSize, BufferUsage.None);
            _vertexData = new byte[_vertexBufferSize * VertSize];
        }
        ...
        _vertexBuffer.SetData(_vertexData, 0, drawData.TotalVtxCount * VertSize);
    ...
    private unsafe void RenderCommandLists(ImDrawDataPtr drawData)
    {
        ...
        for (int n = 0; n < drawData.CmdListsCount; n++)
        {
            ...
            for (int cmdi = 0; cmdi < cmdList.CmdBuffer.Size; cmdi++)
            {
                ...
                if (!_loadedTextures.ContainsKey(drawCmd.TextureId.Handle))
                {
                ...
                var effect = UpdateEffect(_loadedTextures[drawCmd.TextureId.Handle]);
